...
* **paths that skills and docs pin are frozen API**: `game/engine.js`, `dev/balance.js` (the balance-lab CLI — moved out of `game/` in #220), `game/test.js` (now a thin shim that delegates to the `test.{geometry,terrain,cards,maps,ai,reports,seams,integration}.js` `node:test` files it was split into — ADR-0003; the path and `node game/test.js` gate are unchanged; `seams`/`integration` are the WoAProto#222 verification tracer — [[testing-seams]]), `dev/balance-report.js`, `dev/claude-plays.js`, `logs/reports/{balance,skirmish,analysis}/<version>/`, and balance-report's `SAVED:`/`BEST_MAP:` stdout lines — moving any requires a same-commit sweep of `.claude/skills/` + `docs/`
