...
* **acceptance criteria are gates, not wishes**: an AC closes only when a test goes red without it; an AC may not reference a surface no ticket builds (name a blocking foundation ticket instead); a ticket with a UI AC carries a screenshot in its PR. Distrust *"already exists as a prototype"* — a `dev/proto/` mock is a design answer, not a built surface. Full rules in [[workflow]] (`docs/workflow.md`)
