...
* 12 terrain pisces, 2 length 3 mountains, 4 length 2 mountains, 2 length 3 forests and 4 length 2 forest.
...
* Select 1 to play  
* Resolve the card (see card actions section.)  you can always resolve a card as a simple attack or reposition. 
...
* Airdrop  
  * Count \- 1x   
  * Place an Infantry on the board to any empty hex.   
* Conscription   
  * Count \- 1x  
...
* Ordered Withdraw  
  * Count \- 1x  
  * Order an Attack; If a tie attacker lives but does not take the hex.  
* Naval  Barrage  
  * Count \- 1x  
  * Remove a trench or woods from any adjacent controlled hex. (Does not need to attack into hex that had trench or woods)  
  * Order and attack.  (Can just be an attack if no trench was removed)  
* Forced March  
  * Count \- 1x  
