...
Each term carries a `_Home_:` pointer — a backticked `file` plus a backticked anchor (a symbol or expression) — to where the concept is **law** — defined, enforced, or (for not-yet-in-code concepts) documented. No line numbers: the anchor is the greppable key, so a line shift is never drift and never churns the pointer. The pointers are machine-checked: `node dev/check-context.js` fails if a home's file is gone, its anchor no longer appears in that file, or any retired alias regressed. Fix a pointer in the same commit that renames its anchor, moves it to another file, or removes it.
...
_Home_: `game/engine/04-skirmish.js` — `wins[winner] >= I.CONFIG.skirmish.matchTarget`
...
_Home_: `dev/db.js` — `card_events` (a card with declined rows but no played row)
