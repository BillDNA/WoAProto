...
- **The server is the standard dev path**: `run-server.command` (Mac) / `run-server.bat` (Windows) / `node game/server.js`. It serves the app, regenerates `content/manifest.js` at boot, enables every save endpoint (maps, decks, mapsets, reports, debug dumps), and records finished skirmishes into `logs/woa.db` (fail-open — play works without `dev/`). Double-clicked `file://` still plays but persists nothing.
