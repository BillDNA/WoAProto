...
- **Refactors ride the golden-diff net.** `node dev/golden.js` is the committed byte-equality net — a deterministic 10/map × Core Six = 60-game transcript hashed to `dev/golden/core-six-60.json`, DB-free (no `[db]` stderr noise) and N-independent; it runs in the gate via `dev/golden.test.js`. A refactor commit must keep it green on top of test.js + smoke.js. A change that legitimately moves the numbers is a rules/AI-strength change, not a refactor — bump `RULES_VERSION` in `game/engine/01-core.js` atomically with the rule-book header and the test-pin updates, then regenerate the net with `node dev/golden.js --write`. (The older manual oracle — capturing `node dev/balance.js 24 normal` / `24 easy` stdout — still works for ad-hoc checks.)
