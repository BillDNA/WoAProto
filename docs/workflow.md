...
- **Acceptance criteria are gates, not wishes** (learned the hard way from #108: nine tracks shipped their data half green while every ticket's UI acceptance criterion — "fills the #91 workbench slot" — closed unmet, because the workbench only existed as a `dev/proto/` mock and no test went red without it). Applies to any ticket regardless of which skill generated it:
  ...
  - **Red-test rule (at close):** no AC is "done" until a test fails without it. An AC that can't name the test that goes red is a design note, not a gate — and usually points at a missing foundation ticket (above). A UI AC guarded only by the golden-diff can't close: the diff passes whether or not the screen exists, so the UI AC needs its own `dev/smoke.js` assertion that fails when the screen is absent.
  - **Screenshot rule (in the PR):** any ticket carrying a UI acceptance criterion must include a screenshot of that UI in its PR. No screenshot ⇒ the UI half is visibly unbuilt ⇒ the PR isn't done. (`chrome --headless --screenshot=...`, per the headless-screenshot line below.)
  - **Trap phrase:** distrust *"already exists as a prototype."* A proto under `dev/proto/` is a design *answer*, never a built *surface* — any spec sentence leaning on a proto "already existing" is silently assuming a foundation ticket that doesn't exist yet.
