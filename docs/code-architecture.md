...
| A game-setting dial (points cap, weight table, stocks, trench count, map hex ceiling) | `Engine.CONFIG` in `game/engine/01-core.js` | the config home + digest; flat exports alias into it — see below |
| A UI guardrail (battalion size band) | `UI_CONFIG` in `game/ui/ui-config.js` | same pattern, UI tier; read by the builders |
...
- `engine/01-core.js … 07-export.js` — the engine as seven classic-script parts. Each is an IIFE sharing the internal namespace `g.WOA_E` (alias `I`); cross-part names go through `I.*` at the call site, never captured at load time — that convention (plus `boot.js` owning all load-time wiring) is what makes filename order the only ordering constraint. Deep detail: [[engine-model]].
  - `01-core` — `RULES_VERSION` (exported as `Engine.VERSION`), content assembly, rng, static data snapshot, and **`Engine.CONFIG`** — the single home for rules-facing game-setting tunables (points cap, the `POINTS` weight table, terrain stock, trench count, the map hex ceiling — enforced by `validateMaps` and the map editor alike). The flat exports (`BATTALION_POINTS_CAP`, `POINTS`, `TERRAIN_STOCK`) are aliases INTO it (one value, two paths); `Engine.configDigest(obj)` is the deterministic fingerprint util both config homes share (`CONFIG.digest`). Adding a tunable is a one-place edit — the digest picks it up. `AI_WEIGHTS` deliberately stays out (lands on this pattern with #241).
