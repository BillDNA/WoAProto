...
- `engine/00-config.js … 07-export.js` — the engine as nine classic-script parts (the `00a-ai-config` config file sits between `00-config` and `01-core`). Each is an IIFE sharing the internal namespace `g.WOA_E` (alias `I`); cross-part names go through `I.*` at the call site, never captured at load time — that convention (plus `boot.js` owning all load-time wiring) is what makes filename order the only ordering constraint. Deep detail: [[engine-model]].
