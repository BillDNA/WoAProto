...
- `test/` — the engine test suite. `test/test.js` (run with `node game/test/test.js`) is a thin shim that `require`s every test file: the subsystem files beside it — `test.{geometry,cards,maps,ai,reports,ui,seams,integration}.js`, each a `node:test` file — plus each house's own `*.test.js`, which lives with the code it covers (`engine/board/terrain/terrain.test.js`). Add a new section to whichever file owns that code. `test.seams.js` + `test.integration.js` are the verification tracer (mechanism-not-value seam + real-path integration gate) — see [[testing-seams]]. Controlled tests pin `TESTMAP` (bare classic board, in `test/test.helpers.js`).
