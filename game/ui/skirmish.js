...
function renderPrompt(){
  ...
  if (!inputLive()){
    el.innerHTML = (APP.mode==='ai' && v.current!==APP.mySide)
      ...
      : APP.mode==='watch'
        ? 'General <b>'+who+'</b> surveys the field… <span class="small" style="color:var(--note);">(you are spectating)</span>'
        : 'Waiting for <b>'+who+'</b>…';
    return;
  }
  if (v.phase === 'choose-card'){
    ...
    el.innerHTML = '<b>'+who+'</b>: choose a card to play. <span class="small" style="color:var(--note);">(played cards are gone for good)</span>' +
      (adv ? ' <span class="small" style="color:var(--amber);">The cause looks lost — <b>Concede</b> (top right) keeps the campaign moving.</span>' : '');
    return;
  }
