/* War of Attrition — ui part: Field Manual + its diagram player.
   ...
   docs/human-instructions/manual-animations-authoring.md.
...
function mpDrawFrame(f){
  ...
  (f.glowSides||[]).forEach(function(sk){
    ...
    svg.appendChild(svgEl('line', { x1:p1[0],y1:p1[1],x2:p2[0],y2:p2[1],
      stroke:'#e8c252','stroke-width':MP_GLOW_SW.side,'stroke-linecap':'round','class':'medge-glow' }));
  });
  if (f.glowTrench){
    ...
    f.glowTrench.dirs.forEach(function(d){
      ...
      svg.appendChild(svgEl('line', { x1:p1[0],y1:p1[1],x2:p2[0],y2:p2[1],
        stroke:'#e8c252','stroke-width':MP_GLOW_SW.trench,'stroke-linecap':'round','class':'medge-glow' }));
    });
  }
