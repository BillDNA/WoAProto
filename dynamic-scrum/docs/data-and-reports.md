...
# Data & reports — where every skirmish lands and how reports flow

The one primer for the V1 data pipeline: per-skirmish persistence, the report tree, and the
...
**Every skirmish from every source lands as a per-skirmish row in `logs/woa.db`** (gitignored, regenerable —
...
| Live play in the browser | `Engine.hooks.onSkirmishEnd` → `POST /api/recordskirmish` (server proxy, **fail-open** — a zipped `game/` without `dev/` answers 501 and play continues) |
| LLM skirmishes (`dev/claude-plays.js`) | direct `dev/db.js` insert |

Schema: `runs` / `skirmishes` / `card_plays` / per-turn `timeline` (from `st.fsTimeline`, populated by
the engine per completed turn; `GET /api/skirmishes` joins it back as a sibling `fs` field per row and
`envelopeFromRow` folds it into the envelope — WOA-037). `skirmishes` also carries
`hexes_red`/`hexes_blue` (hex-ownership at skirmish end, WOA-038 — NULL on pre-038 rows, excluded from
...
`runs` row but ZERO skirmish rows — the funnel table above overstates it until fixed.
...
`skirmishes` rows carry a `trace` JSON column: the SPEC §4 envelope
`{v,map,seed,fp,winner,winType,turns,trace:[per-play {t,s,a,h,k,ld,u}…],units:{<type>:{dep,atk,abs,kill,die}}}`
(~4 KB/skirmish; unit keys are full names `infantry`/`cavalry`/`artillery`). Gotcha for folds: mixed
...
§7 run). Server reads: `GET /api/runs` (list) and `GET /api/skirmishes?run=<id>` (rows incl. trace),
both guarded — no `dev/db.js` → empty, play continues.

## Reserve-held-at-end (WOA-016)

Per side, how much of that side's pieces were still undeployed (in reserve) when the skirmish ended —
`reserveEndRed`/`reserveEndBlue` in the balance aggregate (a 0..1 share per skirmish, summed like every
other aggregate field), `res_end_red`/`res_end_blue` (raw piece counts) in the `skirmishes` table. It's the
...
Markdown reports are the durable artifact; the DB is the queryable index over the same skirmishes.
...
- `logs/reports/skirmish/<version>/` — claude-plays transcripts (one .md per run, Typicality footer)
  + crash-safe per-skirmish JSONL.
...
- **Every report ends with a tag footer** `#reports #<kind> #v<version, dots as dashes>` (e.g.
  ...
  `claude-plays.js` (skirmish); the review-reports skill writes it on analyses. Search by tag to find the
...
3. The in-browser Balance Dashboard (metrics-v2 Phase 1, WOA-034/035): pill nav **Overview | Maps |
   ...
   pacing minis, all folded from saved skirmish rows (`WOA_REPORT.foldSkirmishes`, same aggregation as the
   ...
   absolute-scale tempo lanes, per-map band board (`ovBandRowHtml` scope='map', 40-skirmish threshold),
   settle curve; `SKIRMISH_CACHE`/`dashLoadSkirmishRows` is the one shared A/B row fetch. Cards/Units are
