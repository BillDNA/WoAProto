# Parking Lot — Meta-progression (deck evolution across a run)

**Status:** direction sketched (Bill's Q.1 loop) · captured 2026-07-15 · graduates to a spec when the reward/upgrade rules are defined

> "How does the player change their deck between battles? operations? campaigns?"
> — Bill, 2026-07-15

**Cadence answered (tentatively) by Bill's Q.1 run loop:** deck changes happen **between operations**, not between individual battles. The tentative loop (see `Questions.md` Q.1 / [[run-design]]):

> commander → **operation** (≈ current first-to-3, term TBD) → play battles → **rewards / update deck / unit upgrades or new unit types?** → next operation → a **campaign** = a series of operations.

So meta-progression = the **between-operation reward-and-deck-update step**. This note develops that step.
