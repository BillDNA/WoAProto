# Graph Report - graphify-setup  (2026-09-05)

## Corpus Check
- 232 files · ~271,385 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1357 nodes · 1940 edges · 205 communities (82 shown, 34 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 136 edges (avg confidence: 0.84)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `292fbf75`
...
5. `Report model (reporting subsystem)` - 13 edges
6. `llmTurn()` - 12 edges
7. `testSkirmish()` - 12 edges
8. `svgEl()` - 12 edges
9. `maybeAI()` - 11 edges
10. `LlmSession()` - 10 edges
...
- `review-reports skill` --semantically_similar_to--> `review-with-rubric skill`  [INFERRED] [semantically similar]
  .claude/skills/review-reports/SKILL.md → .claude/skills/review-with-rubric/SKILL.md
...
## Communities (205 total, 34 thin omitted)
...
Cohesion: 0.05
Nodes (44): assert, E, fs, LAB, { LlmSession }, path, { test }, buildPrompt() (+36 more)

### Community 5 - "claude-plays.js"
Cohesion: 0.07
Nodes (44): ARGS, cap(), cardOptions(), CHOICE_SCHEMA, describeChoice(), DIRN, E, feltNotes() (+36 more)
...
Cohesion: 0.20
Nodes (23): bpAttackLayer(), bpAttackPill(), bpBeginBoard(), bpCoordLabel(), bpEdgeHitLine(), bpEdgePts(), bpGhostHex(), bpHexPoly() (+15 more)
...
Cohesion: 0.18
Nodes (20): boardHexes(), buildGeo(), buildShape(), addHex(), buildTerrain(), coordOf(), dirBetween(), dist() (+12 more)
...
Cohesion: 0.12
Nodes (14): chBindHits(), chCdf(), chEsc(), chHatchDefs(), chLine(), chMakePlacer(), chPolyline(), chSettleSvg() (+6 more)
...
Cohesion: 0.28
Nodes (7): fresh(), testSkirmish(), assert, burnGame(), { E, TESTMAP, testSkirmish }, openerHandSize(), { test }
...
### Community 161 - "terrain-marks.test.js"
Cohesion: 0.18
...
### Community 163 - "Engine model (rules kernel)"
Cohesion: 0.22
...
### Community 171 - "UI invariants (browser app)"
Cohesion: 0.33
...
### Community 176 - "test.commanders.js"
Cohesion: 0.33
Nodes (4): assert, { E, TESTMAP, testSkirmish }, fightSkirmish(), { test }
...
## Knowledge Gaps
- **320 isolated node(s):** `fs`, `os`, `path`, `{ test }`, `assert` (+315 more)
...
- **Why does `Code architecture — start here (front door)` connect `Code architecture — start here (front door)` to `Report model (reporting subsystem)`, `War of Attrition Rule Book (v1.1)`, `Engine model (rules kernel)`, `AI heuristic model`, `Game concepts`, `Commander schema`, `UI invariants (browser app)`, `WarOfAttrition project doctrine (CLAUDE.md)`, `Balance target bands & north stars`, `Hand-off seams & real-path gates`?**
