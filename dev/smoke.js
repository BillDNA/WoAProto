  ...
  function done() {
    ...
    function watchMode() {
      ...
      ok(doc.getElementById('dashRunA').disabled && doc.getElementById('dashRunA').options[0].textContent === '(no server)',
        'file:// — run pickers show the no-server fallback (dashLoadRuns never calls fetch)');
      ...
      ok(doc.getElementById('dashPaneOverview').style.display !== 'none' &&
        ...
        'Overview pane: file:// "no server, in-memory run only" fallback note');
    ...
    // the Overview screen on a SEEDED DASH state — jsdom has no real server, so
    // this stubs win.fetch (GET /api/skirmishes?run=<id>) and win.canNet the way
    // a real browser+server would answer, seeds two tiny fixture runs straight
    // onto DASH.runs/runA/runB, and drives renderOverview through the same
    // #dashPills click every other pane test above uses. Last use of DASH/canNet
    // in this file, so nothing is restored afterward.
