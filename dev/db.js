/* dev/db.js — SQLite persistence for per-skirmish data (V1 data persistence,
   ...
   Usage:
     ...
     var runId = db.insertRun(h, { version: E.VERSION, kind: 'balance',
       ...
       baseline: false });
     db.insertSkirmish(h, runId, st, firstPlayer); // + card_plays + timeline rows
     db.close(h);
...
var SCHEMA = [
  ...
  'CREATE INDEX IF NOT EXISTS idx_card_plays_skirmish ON card_plays(skirmish_id);',
...
// prior skirmish history instead of forcing a delete-and-regenerate.
...
function migrateBattleNames(db) {
  var tbls = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all().map(function (r) { return r.name; });
  if (tbls.indexOf('battles') >= 0 && tbls.indexOf('skirmishes') < 0) {
    db.exec('ALTER TABLE battles RENAME TO skirmishes;');
    ['card_plays', 'timeline'].forEach(function (t) {
      var cols = db.prepare('PRAGMA table_info(' + t + ')').all().map(function (c) { return c.name; });
      if (cols.indexOf('battle_id') >= 0 && cols.indexOf('skirmish_id') < 0) {
        db.exec('ALTER TABLE ' + t + ' RENAME COLUMN battle_id TO skirmish_id;');
      }
    });
    // Drop the stale old-named indexes; SCHEMA recreates the skirmish-named ones.
...
/* WOA-041: the st fields insertSkirmish actually reads, picked into a plain
   ...
   as slimSkirmishState(st), not the whole engine state (board/deck/journal
   would bloat the pipe for nothing). Kept HERE, next to insertSkirmish, so the
...
function listSkirmishes(h, runId) {
  return h.stmts.listSkirmishes.all(runId);
}
