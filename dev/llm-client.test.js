/* dev/llm-client.test.js — unit tests for the pure halves of the claude -p transport.
   Run: node --test dev/llm-client.test.js  (node:test + node:assert, no framework) */
'use strict';
...
/* ---- buildPrompt ---- */
test('buildPrompt: no schema -> user message verbatim', function () {
...
test('buildPrompt: null request -> empty string, no throw', function () {
...
test('parseEnvelope: happy path', function () {
  assert.deepStrictEqual(parseEnvelope(HAPPY, 0),
    { text: 'Blue.', inputTokens: 10, outputTokens: 318, finishReason: 'stop' });
});
test('parseEnvelope: stop_reason max_tokens passes through', function () {
...
test('parseEnvelope: missing usage -> zero tokens, still ok', function () {
...
test('parseEnvelope: is_error true -> errored', function () {
  assert.deepStrictEqual(parseEnvelope(JSON.stringify({ is_error: true, result: 'oops' }), 0), ERRORED);
});
test('parseEnvelope: garbage stdout -> errored', function () {
  assert.deepStrictEqual(parseEnvelope('not json at all {', 0), ERRORED);
});
test('parseEnvelope: empty / whitespace stdout -> errored', function () {
...
test('parseEnvelope: nonzero exit -> errored even with a valid envelope', function () {
  assert.deepStrictEqual(parseEnvelope(HAPPY, 1), ERRORED);
});
test('parseEnvelope: result missing / not a string -> errored', function () {
