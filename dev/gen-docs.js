...
/* dev/gen-docs.js — regenerate the drift-prone doc tables from the code/data
   ...
   Rewrites, IN PLACE, the text between explicit marker pairs:
     ...
     <!-- GEN:content -->        the current content map library (maps with shape +
                                 custom flag, battalions, mapsets with the active
...
var WEIGHT_DESC = {
  attrWin: 'Huge swing for being the projected attrition winner if the battalions ran out now. Ramps up as cards run low. This is the anti-stalemate term — **don\'t zero it.**',
  ...
  unitOnBoard: 'Value of each of my deployed units (× that type\'s own `unitValue` price).',
  unitValue: 'What the search pays for one piece of each type, keyed by unit type. Installed by the unit house (`game/engine/unit/unit-config.js`); a personality or Commander may override any row.',
...
function genContent() {
  ...
  var battalions = (globalThis.WOA_CONTENT.battalions || []).map(function (d) {
  ...
  var units = ((globalThis.WOA_CONTENT || {}).units || []).map(function (u) {
    var tot = Object.keys(u.units || {}).reduce(function (s, t) { return s + (u.units[t].count || 0); }, 0);
    return '`' + u.id + '` (' + tot + ' pieces' + (u.active ? ', ACTIVE' : u.experimental ? ', experimental' : '') + ')';
  });
  var commanders = (E.COMMANDERS || []).map(function (c) {
    var strengths = (c.traits || []).filter(function (t) { return t.role !== 'weakness'; }).length;
    var weaknesses = (c.traits || []).filter(function (t) { return t.role === 'weakness'; }).length;
    return '`' + c.id + '` (' + strengths + ' strength' + (strengths === 1 ? '' : 's') + ' / ' + weaknesses + ' weakness' + (weaknesses === 1 ? '' : 'es') + ')';
  });
  return [
    '_Generated by `node dev/gen-docs.js` — rerun it after content changes._',
    '',
    '- **Maps (' + maps.length + '):** ' + maps.join(', '),
    '- **Battalions (' + battalions.length + '):** ' + battalions.join(', '),
    '- **Mapsets (' + sets.length + '):** ' + sets.join(', '),
    '- **Unit-sets (' + units.length + '):** ' + (units.join(', ') || '_none — engine uses the maps.js default 7/2/1_'),
    '- **Commanders (' + commanders.length + '):** ' + (commanders.join(', ') || '_none_')
  ].join('\n');
}
